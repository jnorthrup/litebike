...
/// Resolves a hostname to an IP address using DoH resolver.
async fn resolve_host(host: &str) -> io::Result<IpAddr> {
    ...
    // Use DoH resolver with fallback to system DNS
    match get_doh_resolver().await.lookup_ip(host).await {
        Ok(response) => {
        ...
        Err(e) => {
            warn!("DoH resolution failed for {}: {}, falling back to system DNS", host, e);
...
/// Handle universal port connections with protocol detection (POSIX optimized with fallback)
async fn handle_universal_connection(stream: TcpStream) -> io::Result<()> {
    // Try POSIX peek first for efficiency, fallback to async detection
    match universal_listener::detect_protocol_posix(&stream) {
        Ok(protocol) => {
            match protocol {
                universal_listener::Protocol::Http => {
                    debug!("Routing to HTTP handler via POSIX detection");
                    handle_http(stream).await
                },
                universal_listener::Protocol::Socks5 => {
                    debug!("Routing to SOCKS5 handler via POSIX detection");
                    handle_socks5(stream).await
                },
                universal_listener::Protocol::Pac => {
                    debug!("Routing to PAC handler via POSIX detection");
                    pac::handle_pac_request(stream).await
                },
                universal_listener::Protocol::WebSocket => {
                    debug!("Routing to WebSocket handler via POSIX detection");
                    handle_http(stream).await // WebSocket upgrade handled by HTTP
                },
                _ => {
                    debug!("Unknown protocol via POSIX detection, defaulting to HTTP");
                    handle_http(stream).await
                }
            }
        }
        Err(_) => {
            // Fallback to standard async detection (consumes bytes)
            let mut stream = stream;
            match universal_listener::detect_protocol(&mut stream).await? {
                (universal_listener::Protocol::Http, consumed_data) => {
                    debug!("Routing to HTTP handler via async detection");
                    let prefixed_stream = universal_listener::PrefixedStream::new(stream, consumed_data);
                    handle_http(prefixed_stream).await
                },
                (universal_listener::Protocol::Socks5, consumed_data) => {
                    debug!("Routing to SOCKS5 handler via async detection");
                    let prefixed_stream = universal_listener::PrefixedStream::new(stream, consumed_data);
                    handle_socks5(prefixed_stream).await
                },
                _ => {
                    debug!("Unknown protocol via async detection, defaulting to HTTP");
                    handle_http(stream).await
                }
            }
        }
