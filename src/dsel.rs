...
// Import keymux DSEL from literbike for quota tracking
use literbike::keymux::dsel::{ProviderQuotaTracking, RuleEngine};
...
fn is_real_key(val: &str) -> bool {
    ...
    if v.starts_with("your_") || v.starts_with("sk-placeholder") {
        return false;
    }
    if v.contains("xxx") || v.contains("here") || v.contains("TODO") {
        return false;
    }
    true
}
...
pub fn is_real_key_pub(val: &str) -> bool {
    is_real_key(val)
}
...
pub fn discover_providers() -> Vec<Provider> {
    ...
    for (priority, (name, key_envs)) in provider_specs.iter().enumerate() {
        ...
        if let Some(key_env) = found {
            ...
            let base_url = env::var(&base_url_env).unwrap_or_else(|_| get_default_base_url(name));
...
pub fn status() -> Vec<(String, String, u8, bool)> {
    ...
    all_names
        .iter()
        .enumerate()
        .map(|(priority, (name, key_envs))| {
            let has_key = key_envs
                .iter()
                .any(|k| env::var(k).ok().map_or(false, |v| is_real_key(&v)));
            let base_url_env = format!("{}_BASE_URL", name.to_uppercase());
            let base_url = env::var(&base_url_env).unwrap_or_else(|_| get_default_base_url(name));
...
pub fn all_provider_quotas() -> Vec<(String, u64, u64, f64)> {
    ...
    quotas
        ...
        .map(|(name, tracking)| {
            (
                name.clone(),
                tracking.tokens_used_today,
                tracking.estimated_remaining_quota,
                tracking.quota_confidence,
            )
        })
        .collect()
}
