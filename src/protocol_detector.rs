...
use crate::static_generation::{get_static_detector, detect_with_autovec};
...
impl ProtocolDetector {
    ...
    /// Detect protocol using static code generation (zero-overhead)
    /// Falls back to dynamic detection if static generation is disabled
    #[cfg(feature = "static-generation")]
    pub fn detect_static(&self, buffer: &[u8]) -> DetectionResult {
        // Use static generation with autovec optimization
        detect_with_autovec(buffer)
    }

    /// Fallback detection when static generation is not available
    #[cfg(not(feature = "static-generation"))]
    pub fn detect_static(&self, buffer: &[u8]) -> DetectionResult {
        // Use the existing dynamic detection
        self.detect(buffer)
    }

    /// Hybrid detection that chooses the best method based on buffer characteristics
    pub fn detect_optimized(&self, buffer: &[u8]) -> DetectionResult {
        if buffer.is_empty() {
            return DetectionResult::unknown();
        }

        // For single byte detection, use static generation if available
        #[cfg(feature = "static-generation")]
        {
            let static_detector = get_static_detector();
            
            // If this byte has low penalty (rare), prefer static detection
            if static_detector.penalty(buffer[0]) <= 2 {
                return static_detector.detect(buffer);
            }
        }

        // For complex patterns or when static generation unavailable,
        // use dynamic detection with rarity ordering
        self.detect(buffer)
    }

    /// Ultra-fast detection using autovec optimizations
    pub fn detect_autovec(&self, buffer: &[u8]) -> DetectionResult {
        // Use CPU-adaptive detection with autovec
        let features = arch_specific::detect_cpu_features();
        arch_specific::adaptive_detect(buffer, features)
    }

    /// Best available detection method (combines all optimizations)
    pub fn detect_best(&self, buffer: &[u8]) -> DetectionResult {
        if buffer.is_empty() {
            return DetectionResult::unknown();
        }

        // For very short buffers, use simple table lookup
        if buffer.len() == 1 {
            let first_byte = buffer[0];
            if self.byte_rarity(first_byte) == 1 {
                // Rare byte - use optimized path
                return self.detect_autovec(buffer);
            }
        }

        // For longer buffers, choose best method based on characteristics
        match buffer.len() {
            1..=3 => {
                // Short buffers: use static generation if available
                #[cfg(feature = "static-generation")]
                {
                    self.detect_static(buffer)
                }
                #[cfg(not(feature = "static-generation"))]
                {
                    self.detect_autovec(buffer)
                }
            }
            4..=16 => {
                // Medium buffers: use autovec optimizations
                self.detect_autovec(buffer)
            }
            _ => {
                // Long buffers: use hybrid approach
                self.detect_optimized(buffer)
            }
        }
    }
}
